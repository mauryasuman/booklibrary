 ...
 * seedBooksAPI.js
 * 
 * Usage: node seedBooksAPI.js
 * 
 * Inserts 40 UNIQUE books into the database by calling the backend API.
 * This works even if the local MongoDB is down (backend uses in-memory DB fallback).
 */

const http = require('http');

// 40 UNIQUE BOOKS DATA
const books = [
  // 1. Fiction
  {
...
async function run() {
  ...
  while (attempts < 30) {
    try {
      const health = await makeRequest('GET', '/api/health'); // /api/health is the correct endpoint
  ...
  if (attempts >= 30) {
    console.log('❌ Server timed out! Please ensure "npm run dev" is running.');
    process.exit(1);
  }
  ...
  for (const book of books) {
    try {
      const res = await makeRequest('POST', '/books', book, {
      ...
      if (res.data.success) {
        ...
        console.log(`⚠️ Failed: ${book.title} - ${res.data.message}`);
      }
    } catch (e) {
      console.log(`❌ Error: ${book.title}`);
    }
  }

  console.log(`\n🎉 Completed! Added ${count} books.`);
